...
void NIF_Packet_Credit::ackMsg() {
	...
	if(parallelFC){
	    sendDelayed(ack,clockPeriod_packet/2,ackOut);
	}else{
...
void NIF_Packet_Credit::netPortMsgArrived(cMessage* msg) {
    ...
    // With parallel flow control, net packets may arrive also from the ack input.
    if (msg->getArrivalGate() == packetPortIn || msg->getArrivalGate() == ackIn) {
