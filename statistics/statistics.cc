...
// Included Wireless Energy.

// Here is where the statistics are "loaded".
// You can decide to omit or to create new statistics and load them here.
...
//	groups.push_back(sg);
	...
	sg = new StatGroup_ListAll("Wireless Energy",
			StatObject::ENERGY_STATIC, StatObject::ENERGY_LAST, "wireless");
	groups.push_back(sg);

	sg = new StatGroup_Sum("Wireless Energy", StatObject::ENERGY_STATIC,
	...
	sg = new StatGroup_ListAll("Performance: Application", StatObject::AVG,
				StatObject::AVG, "application");
	groups.push_back(sg);

	sg = new StatGroup_ListAll("Traffic Characteristics", StatObject::TIME_AVG,
	                StatObject::HURST, "trafficGen");
	groups.push_back(sg);

	sg = new StatGroup_ListAll("Performance: Communication", StatObject::TIME_AVG,
						StatObject::AVG, "NIF");
	groups.push_back(sg);
...
//	groups.push_back(sg);
	...
	// Canviar MMA -> AVG
//	sg = new StatGroup_ListAll("Performance: Wireless Comm", StatObject::MMA,
//				StatObject::TIME_AVG, "wNIF");
//	groups.push_back(sg);
...
void Statistics::handleMessage(cMessage* msg) {
    ...
    if(strcmp(msg->getName(),"warmup")==0) {
        warmup = msg->getTimestamp();
        StatObject::warmup = SIMTIME_DBL(warmup);
    } else if(strcmp(msg->getName(),"cooldown")==0) {
        cooldown = msg->getTimestamp();
        StatObject::cooldown = SIMTIME_DBL(cooldown);
    } else {
        opp_error("Unexpected message arrived to the statistics module.");
    }
...
StatObject* Statistics::registerStat(string name, int type, string modifiers) {
	...
	case StatObject::TOTAL:
		...
		break;
	case StatObject::AVG:
	    ret = new StatObject_Avg(name);
	    break;

	case StatObject::HURST:
	    ret = new StatObject_HURST(name);
	    break;

	case StatObject::STATIC_TOTAL:
	    ret = new StatObject_StaticTotal(name);
	    break;
