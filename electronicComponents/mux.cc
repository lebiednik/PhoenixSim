...
void mux::handleMessage(cMessage *msg) {
	...
	if(msg == qMessage){
		...
		if(transChannel->isBusy()) {
		    simtime_t noww = simTime();
		    simtime_t finishtimee = transChannel->getTransmissionFinishTime();
		    if((finishtimee - noww) < clockPeriod)
		           scheduleAt(simTime()+clockPeriod, qMessage);
		    else
		           scheduleAt(finishtimee, qMessage);
		} else
		...
		if (transChannel->isBusy() || qMessage->isScheduled() || timerMessage->isScheduled()) {
			...
			if(!qMessage->isScheduled() && !timerMessage->isScheduled()) {
			    ...
			    simtime_t finishtime = transChannel->getTransmissionFinishTime();
			    if((finishtime - now) < clockPeriod)
			        scheduleAt(simTime()+clockPeriod, qMessage);
			    else
			        scheduleAt(finishtime, qMessage);
			}
		} else
			send(msg, outgate);
	} else
