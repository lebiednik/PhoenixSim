...
void NIF::initialize() {
	...
	SO_bw = Statistics::registerStat("Bandwidth eNIF Out (Gb/s)", StatObject::TIME_AVG, "eNIF");

	SO_qTime
			= Statistics::registerStat("Time in eNIF Q", StatObject::AVG, "eNIF");
...
void NIF::handleMessage(cMessage* msg) {
	...
	//} else if (arrivalGate == ackIn){
	//    BufferAckMsg* bamsg = check_and_cast<BufferAckMsg*> (msg);
	//    ackMsgArrived(bamsg);
	//    // WARNING: using parallelFC with ElectronicCC may not work
...
void NIF::requestMsgArrived(RouterCntrlMsg* rmsg) {
	...
	if (rmsg->getType() == proc_grant) {
		...
		simtime_t ser_delay = (simtime_t)clockPeriod_packet*ceil((double)toProcQ.front()->getSize()/flitWidth);
		sendDelayed(toProcQ.front(), ser_delay, toProc);
