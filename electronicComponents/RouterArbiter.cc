...
#include "electronicComponents/RouterArbiter.h"
#include "statistics.h"

#include "Arbiter_PR.h"

#include "Arbiter_EM.h"
#include "Arbiter_EMB.h" // NEW
...
void RouterArbiter::initialize(int stage) {
	...
	if (arbiter != NULL) {
		arbiter->init(myId, level, numX, numY, numZ, numVC, numPorts, numPSE,
		...
		for (int i = 0; i < numPorts; i++) {
			if (msg->arrivedOn(reqIn[i]->getId())) {
				...
				if (req->getType() == router_bufferAck) {
					...
					arbiter->unblock(unb->getToPort(), unb->getFromPort());
					delete msg;
			...
			} else if(msg->arrivedOn(ackIn[i]->getId())) {
			    ...
			    delete msg;
				    ...
				    opp_error("ERROR: non ack arrived at arbiter's ackport");
				}
