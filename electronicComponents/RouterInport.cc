...
void RouterInport::initialize() {
	...
	E_dynamic = Statistics::registerStat(statName, StatObject::ENERGY_EVENT,
			"electronic");
	injected = Statistics::registerStat("Injected flits", StatObject::TOTAL,
	            "electronic");
				...
				grant->getType() == router_grant_destroy)
		{
			...
			else
			{
				...
				if (grant->getType() != router_grant_destroy)
				{
	                ...
	                // En un broadcast, aquest és l'últim flit que es serveix
					emsg->setSourceBit(grant->getSourceBit());
					emsg->setLTB(grant->getLTB());
					emsg->setRTB(grant->getRTB());
...
void RouterInport::sendRequest(ElectronicMessage* emsg, int vc) {
	...
	if(type == 26) { // FBFLY
	    del = 2*(int)ceil( (double)log10(numPorts-3)/ (double)log10(4) )-1 ;
	}

	sendDelayed(req, del*clockPeriod, reqportOut);
